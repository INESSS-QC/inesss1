...
stat_gen1_args <- replace_NA_in_dt(list_as_dt_no_recycled(list(
  METHODE = ll$METHODE, DATE_DEBUT = ll$DATE_DEBUT, DATE_FIN = ll$DATE_FIN,
  TYPE_RX = ll$TYPE_RX, CODE_RX = ll$CODE_RX,
  CODE_SERV_FILTRE = ll$CODE_SERV_FILTRE, CODE_SERV = ll$CODE_SERV
)), by = "")
...
stat_gen1_args <- replace_NA_in_dt(list_as_dt_no_recycled(list(
  METHODE = ll$METHODE, DATE_DEBUT = ll$DATE_DEBUT, DATE_FIN = ll$DATE_FIN,
  TYPE_RX = ll$TYPE_RX, CODE_RX = ll$CODE_RX, GROUPER_PAR = ll$GROUPER_PAR,
  CODE_SERV_FILTRE = ll$CODE_SERV_FILTRE, CODE_SERV = ll$CODE_SERV,
  CODE_LIST_FILTRE = ll$CODE_LIST_FILTRE, CODE_LIST = ll$CODE_LIST
)), by = "")
kable_tab(stat_gen1_args)
```
...
`"Exclusion"` (ou `"Inclusion"`) des codes de services [`SMED_COD_SERV`] et `"Inclusion"` (ou `"Exclusion"`) des codes de catégorie de listes de médicaments [`SMED_COD_CATG_LISTE_MED`].
...
* **`Type de Code Rx` :** `DENOM` ou `DIN`. Indique le type de `Code Rx` à analyser.
  ...
  + `DIN` : SMED_COD_DIN.
* **`Code Rx X` :** Le(s) code(s) de médicament à analyser.
* **`Grouper par` :**
  + `Périodes` : Permet de totaliser les statistiques des `Codes Rx` pour chaque période d'étude.
...
* **Exécuter Requête :** Exécution d'une requête SQL à partir des arguments sélectionnés. Une fois complétée, le tableau des résultats s'affiche sous le bouton.
