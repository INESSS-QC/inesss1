...
# Requêtes via Excel
Exécution d'une ou plusieurs requêtes par l'intermédiaire d'un fichier Excel. Chaque onglet doit contenir un tableau indiquant les arguments d'une méthode. Il est fortement conseillé d'utiliser [les gabarits disponibles](https://github.com/INESSS-QC/inesss1/tree/master/Documentation).
...
3. Si la structure est acceptée, le message `Aucune erreur, exécution possible.` s'affiche. Sinon, le message affiche toutes les erreurs à corriger par onglet.
...
* **`RESULTATS_PAR` :** Regrouper les résultats par :
...
* **`DENOM / DIN` :** Code(s) analysé(s) à l'intérieur de la période d'étude. Si les résultats sont regroupés par période d'étude, cette colonne se situera à la fin du tableau.
...
* **`TENEUR` :** Teneur du médicament. *Conditionnel* : Résultats par `"Teneur"`.
* **`FORMAT_ACQ` :** Format d'acquisition du médicament. *Conditionnel* : Résultats par `"Format"`.
...
## Arguments
Inscription des arguments de la requête SQL par l'utilisateur.
...
* **Bouton Exécuter Requête :** Exécution d'une requête SQL à partir des arguments sélectionnés. Une fois complété, le tableau des résultats s'affiche sous la section *Résultats*.
* **Bouton Réinitialiser Arguments :** Réinitialisation complète de la page (arguments par défaut comme lors de la première connexion).
...
## Requête SQL
Code SQL qui a été utilisé pour créer le tableau des résultats en fonction des arguments sélectionnés. Seul le code de la première période est affiché.
