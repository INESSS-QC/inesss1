# remotes::install_github("INESSS-QC/inesss", upgrade = "never", build_vignettes = T,force = TRUE)
rm(list = ls())

setwd("V:/GI/Projets/4.Projets_en_cours/Impact_Covid_Cardiologie/L2_Coh_Cardio/Data")
...
conn <- SQL_connexion("ms026a")  # connexion à Teradata
debut <- "2006-04-01"  # debut de la periode d'etude
...
#SQL_Dx <- list(
#  coronarien = list(CIM9 = paste0(410:414, "%"),
#                   CIM10 = paste0("I", 20:25, "%"))
#)

# Insuffisance cardiaque
SQL_Dx <- list(
  InsufCardiaq = list(CIM9 = paste0(428, "%"),
                    CIM10 = paste0("I", 50, "%"))
)
...
#Dx_etape2 <- Dx_etape2_autre[,.(ID, DATE_DX)]
...
dt_cond_med<-dt_final %>% 
  dplyr::filter(!is.na(DI_Finale)) %>% 
  dplyr:: select(ID,D_Recent,DI_Finale)

# sauvegarder sur le V:\
# Maladie coronarienne
# write.csv(dt_cond_med,"dt_cond_med_20210621_V2juin.csv",row.names = F)
# Insuffisance cardiaque
write.csv(dt_cond_med,"InsufCardiaq_20060101_20210622_V2juin.csv",row.names = F)

# rm(list = ls())


# test<-Dx_etape2 %>% 
#  filter(ID=="inscrire un id")
