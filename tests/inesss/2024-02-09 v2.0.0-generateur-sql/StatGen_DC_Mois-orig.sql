...
DATE '2024-01-31' AS maxDate, -INTERVAL '12' MONTH + (maxDate + INTERVAL '1' DAY) AS minDate),
...
EXTRACT(YEAR FROM SMED_DAT_SERV) AS annee_civile,	
EXTRACT(MONTH FROM SMED_DAT_SERV) AS mois,
...
LEFT JOIN ben
ON temp.periode_ref = ben.periode_ref AND temp.groupe_age = ben.groupe_age AND temp.annee_civile = ben.annee_civile AND temp.mois = ben.mois)						
