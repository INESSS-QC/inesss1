WITH dat AS (
	SELECT
	DATE '2024-01-31' AS maxDate,
	-INTERVAL '1' YEAR + (maxDate + INTERVAL '1' DAY) AS minDate
),
denom AS (
	SELECT DISTINCT(VAL_COD) AS dco
	FROM cod_denom_comne
	WHERE VAL_COD IN (46088, 46342, 46454, 46488, 46580, 47017, 47226, 47284, 47300, 47338, 47483, 47548, 47736)
),
cat AS (
SELECT liste
FROM	(	SELECT 3 AS liste FROM (SELECT 1 AS "DUMMY") AS "DUAL"
			UNION
			SELECT 40 AS liste FROM (SELECT 1 AS "DUMMY") AS "DUAL"
			UNION
			SELECT 41 AS liste FROM (SELECT 1 AS "DUMMY") AS "DUAL"
		) AS tab
),
temp AS (
	SELECT
...
nom AS (
	...
	FROM temp
		...
		LEFT JOIN ben ON temp.periode_ref = ben.periode_ref AND temp.groupe_age = ben.groupe_age
)
SELECT
	groupe_age,
