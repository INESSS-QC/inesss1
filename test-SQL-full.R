library(inesss)
library(data.table)
dt = as.data.table(readRDS("V:/_MedPersAg/Charlson_test.rds"))
setkey(dt)
dt <- unique(dt)
t1 <- Sys.time()
TEST <- SQL_comorbidity(
  conn = SQL_connexion("ms045"), uid = NULL, pwd = NULL,
  dt = dt, ID = "ID", DATE_INDEX = "DAT_Index",
  ...
  exclu = list(obstetric_diab = TRUE, obstetric_hyper = TRUE),
  verbose = TRUE
)
t2 <- Sys.time()
