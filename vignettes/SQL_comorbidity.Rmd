...
# Introduction
Les exemples ci-dessous utilisent le code de la fonction `SQL_comorbidity` sur des tables fictives pour montrer la méthode utilisée.
...
* Sélectionner la première date indexe (la plus ancienne) s'il y en a plusieurs pour un même individu.
...
## CIM = c('CIM9', 'CIM10')
Il est possible de choisir entre les code CIM9, CIM10 ou les deux. Ici on sélectionne les deux types de codes et on va les chercher dans toutes les bases de données `dt_source`. Voici un aperçu des codes recherchés des trois premiers éléments :
```{r}
...
**Rappel dates indexes**
```{r}
dt
```
Considérant que :

1. Les dates indexes sont comprises entre le 2020-01-01 et le 2020-12-31.
2. `lookup = 2` ans. 
...
Cette extraction implique qu'un individu ayant une date indexe le 2020-01-01 pourrait avoir des dates de diagnostics comprises entre le 2020-01-01 et le 2020-12-31. Puisqu'on cherche des dates antérieures à la date indexe, les dates supérieures à 2020-01-01 doivent être exclus de l'analyse. De la même manière, un individu ayant une date indexe le 2020-12-31 pourrait avoir des dates inférieures à `2020-12-31 - 2ans - 730jours`.

Puisque les dates inférieures à `DATE_INDEX - lookup` servent uniquement à confirmer un autre diagnostics, les diagnostics où `confirm_sourc = 1` et la date est plus petite que `DATE_INDEX - lookup` ne seront pas utilisés, donc supprimé de la table d'analyse.
...
1. La ligne 1 sera supprimée, car `confirm_sourc(MEDECHO = 1)` et `DATE_DX < DATE_INDEX - lookup`.
...
# Exclusion Cas Obstétriques (`obstetric_exclu = TRUE`)
...
```{r}
Obstetrics_diagn_codes$obstetric
```
...
Si différentes sources indique un même diagnostic et une même date, on priorise la source où `confirm_sourc` égale 1, puis 2.
```{r, echo=TRUE}
...
# Dx < DATE_DX $-$ lookup
La table d'analyse peut contenir des dates qui sont inférieures à la période d'étude. Si la période d'étude est entre le 2018-01-01 et le 2019-12-31, on ne veut pas qu'une date en 2017 confirme une autre date en 2017, on veut plutôt que les dates en 2017 confirment celles en 2018 et 2019.  
On va donc regarder en 2018 et 2019 quelles sont les dates qui sont confirmées par une date qui se situe avant la période d'étude. Dans un but d'optimisation, seule la première date qui confirme une autre date sera conservée.
...
# Conformation des diagnostics
Si l'élément de `confirm_sourc` égale 2 (`BDCU` et `SMOD`), le diagnostic provenant de cette source a besoin d'être confirmé par une date précédente. Ces sources peuvent se confirmer entre elles.
