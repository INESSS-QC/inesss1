...
1. Télécharger sur votre poste le dossier d'installation à partir de Teams ou du J:.
...
# Installer la librairie remotes au besoin :
if (!"remotes" %in% installed.packages()[,1]) install.packages("remotes")

# Installation de la librairie R inesss
remotes::install_local("Inscrire/le/répertoire/du/dossier/R_inesss_1.x.y.tar.gz", upgrade = "never")
```

**Exemple**
```{r, eval=FALSE, echo=TRUE}
remotes::install_local("E:/Desktop/MonDossier/Rpackages/R_inesss_1.x.y.tar.gz", upgrade = "never")
```
